/**
 * Waiver Page — induction video + digital signing flow
 */
...
/**
 * Open waiver signing flow for a member
 * Called from member detail or check-in
 */
async function openWaiverFlow(memberId, type = null) {
  if (typeof window.api === 'undefined') return;

  const member = await window.api.member.getById(memberId);
  if (!member) { showToast('Member not found', 'error'); return; }

  waiverCurrentMember = member;

  // Determine type: if member is minor, use minor form. Otherwise adult.
  if (!type) {
    type = member.is_minor ? 'minor' : 'adult';
  }

  const template = await window.api.waiver.getActiveTemplate(type);
  ...
  showModal(`
    <div class="p-6 max-h-[85vh] overflow-y-auto">
      ...
      <form id="waiver-form" onsubmit="submitWaiver(event)">
        <!-- Conditions of use (read-only display) -->
        ...
        ${isMinor ? `
          <!-- Minor-specific: Photo ID consent -->
          <div class="space-y-3 mb-6">
...
async function submitWaiver(e) {
  ...
  // Collect dependents for minor form
  let dependents = null;
  if (waiverCurrentTemplate.type === 'minor') {
    dependents = [];
  ...
  // Update member with emergency contact and medical info
  await window.api.member.update(waiverCurrentMember.id, {
