...
- The `crux-app.service` sometimes fails to start because port 8080 is already in use (old nohup process)
- Fix: add `ExecStartPre=/bin/bash -c 'fuser -k 8080/tcp || true'` to the systemd service file
- Workaround until fixed: `sudo fuser -k 8080/tcp && sudo systemctl restart crux-app`

### 2. End-to-end test — full happy path
Work through the entire flow as a real user would. Fix anything that breaks.

**Signup → setup:**
- [ ] Go to `cruxgym.co.uk/signup` — loads, form works, monthly/annual toggle works
...
- [ ] Register a member via `/register` (full 5-step flow: video → type → details → waiver → sign)
- [ ] Portal invite email arrives after registration, `/me` login works
...
**Member portal (`/me`):**
- [ ] OTP login works — enter email, receive code, log in
- [ ] Home tab: QR code shows, fullscreen tap works, grade pyramid shows
- [ ] Map tab: gym map loads, routes show as pins
- [ ] Logbook: mark a route as sent, appears in logbook
- [ ] Noticeboard: post from staff app, appears in member portal
...
**Emails:**
- [ ] Welcome email — arrives, links to correct subdomain
- [ ] Portal invite after registration — arrives, link works
- [ ] Staff invite email — arrives, link works

### 3. ✅ Member Portal — BUILT
- OTP email auth → JWT session
- Home: QR code (fullscreen), pass status, grade pyramid, "Buy a Pass" link
- Map + routes (read-only, zoomable SVG)
- Logbook: mark routes as sent/unsent
- Noticeboard: gym announcements
- Per-gym accent colour + shop URL (Settings → General → Member Portal)
- Portal invite sent automatically on registration + manually from staff member profile

### 4. ✅ Integration — DONE
- Marketing site (`cruxgym.co.uk`) nav has "Start Free Trial" → `/signup` and "Log In" links
- All hero, pricing, and footer CTAs link to `/signup`
- Portal invite included in registration flow
- Member registration is a 5-step public flow at `/register`

### 5. Email sequence — automated onboarding
...
- Implementation: cron/interval on server startup querying `platform.db` for gyms near/past trial end

### 6. Super-admin improvements
- ✅ Stats strip (total gyms, members, active, trialing, suspended, revenue)
- ✅ Quick links to every part of the platform (staff login, member portal, register per gym)
- ✅ Iron Man/JARVIS theme
- [ ] "Open Gym" impersonate button — one-time token → redirect to gym as Owner
- [ ] Revenue dashboard: MRR, trial-ending-soon list

### 7. ✅ Production hardening — DONE
- ✅ Real `JWT_SECRET` set in `/etc/crux.env`
- ✅ Real `ADMIN_TOKEN` set in `/etc/crux.env`
- ✅ `SMTP_PASS` in env file (not hardcoded)
- ✅ Rate limiting on `/api/me/auth` (20 req/15min)
- ✅ Monthly + annual Stripe price IDs all set
- [ ] Confirm Cloudflare SSL mode is "Full" not "Flexible"
- [ ] Input validation on public-facing routes (signup, register, me)
- [ ] Rename `boulderryn-project/` → `crux/` on EC2

### 8. Monitoring & backups
- ✅ Nightly DB backup — `scripts/backup-dbs.sh` via cron, 14-day retention, `/home/ec2-user/crux-backups/`
- [ ] UptimeRobot (free) on `cruxgym.co.uk` + EC2 IP — alerts Oscar if server goes down
- [ ] S3 offsite backup (longer term — currently local only)

### 9. Branded email templates
- [ ] Consistent HTML template: Crux logo, navy/white, footer
